...
export class FloorItemComponent implements OnInit {
  ...
  displayedColumns: string[] = ['floor', 'floorItem','normalQuantity','maximumQuantity','usedQuantity','floorItemStatus','action'];
...
;
  ...
  initiateForm(){
      this.form = this.fb.group({
        ...
        floorItemStatusReport:['', [Validators.required]],
  ...
  update(data:FloorItemData){
    ...
    this.form.patchValue({
      floorItemNormalQuantity:data.floorItemNormalQuantity,
        ...
        floorItemItemId:data.floorItem.id,
        floorItemProjectFloorId:data.floorItemProjFloor.id
  });
  }
  ...
  getProjectFloors(){
    this.projectFloorService.getProjectFloorsList(null).subscribe(res=>{
      this.projectFloorsList= res['content'];
    });
  }
  ...
  //   this.orgService.createFloorItem(floorItem).subscribe(res => {
  ...
  delete(data:FloorItemData){
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
          title: "Are you sure?",
          message: "You are about to delete floor item: "+data.floorItemProjFloor.pFloorValue}
    });
  
    // listen to response
    dialogRef.afterClosed().subscribe(res => {
      if(res){
        this.floorItemService.deleteFloorItem(data.id).subscribe(res => {
